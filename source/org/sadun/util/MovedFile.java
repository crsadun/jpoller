 ...
 * <p>
 * The class provides an auto-synchronization mode (disable by default)
 * which checks the paths with actual files on the filesystem
 * before performing an operation.
 * <p>
 * The check will fail, however, if files for both original and destination
 * path do exist (i.e. if a copy has taken place).
 *
 * @author C. Sadun
 * @version 1.0
 */
public class MovedFile {
    ...
    /**
     * Create a MovedFile object, with the given original and destination
     * paths, and whose state is the given one.
     * @param original the original path for the move operation
     * @param destination the destination path for the move operation
     * @param isOriginal <b>true</b> if the move operation has not occurred yet,
     *                   <b>false</b> otherwise
     */
    ...
    /**
     * Create a MovedFile object, with the given original and destination
     * paths, and whose state is automatically detected.
     * @param original the original path for the move operation
     * @param destination the destination path for the move operation
     */
    ...
    /**
     * Invert the move operation. If the file had been moved, it's restored
     * in its original position, and viceversa.
    ...
    /**
     * Revert the move operation. If the file had been moved, it's restored
     * in its original position. If it's already reverted, a RuntimeException
     * will be raised.
     * @exception RuntimeException if the file is in wrong state
     * @return <b>true</b> if the operation is successful.
     */
    ...
    /**
     * Perform again the move operation. If the file had been reverted, it's moved
     * in its previous "moved" position. If it's already moved, a RuntimeException
     * will be raised.
     * @exception RuntimeException if the file is in wrong state
     * @return <b>true</b> if the operation is successful.
     */
     ...
     * @exception RuntimeException if a filesystem file exists for both original
    ...
    /**
     * Return the destination path of the file. Since the file has been moved,
     * this File object corresponds to the physical file on
     * the filesystem.
     * @return the destination path of the file.
     */
    public File getDestinationPath() { return destination; }
