...
class TestPollManager extends BasePollManager {
    ...
    public void cycleStarted(CycleStartEvent evt) { System.out.println("Poller awakened "+(evt.getPoller().getFilter()==null ? "(no filtering)" : evt.getPoller().getFilter().toString())); }
     public void cycleEnded(CycleEndEvent evt) { System.out.println("Poller going to sleep");  }
     public void directoryLookupStarted(DirectoryLookupStartEvent evt) { System.out.println("Scanning "+evt.getDirectory());  }
     public void directoryLookupEnded(DirectoryLookupEndEvent evt) { System.out.println("Finished scanning "+evt.getDirectory());  }
     ...
     public void exceptionDeletingTargetFile(File target) { System.out.println("Exception deleting "+target);  }
     public void exceptionMovingFile(File file, File dest) { System.out.println("Could not move "+file+" to "+dest);  }
     
     public String toString() { return "TestPollManager - a pollmanager which just echoes the received events"; }
